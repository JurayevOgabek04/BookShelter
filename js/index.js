...
const getBooks = async function (kniga, page, sort = "relevance") {
  const request = await fetch(
    `https://www.googleapis.com/books/v1/volumes?q=${kniga}&startIndex=${(page - 1) * 10 + 1}&orderBy=${sort}`
  );
...
elSortBtn.addEventListener("click", evt => {
  if (evt.target.matches(".sort__btn")) {
    sort = "newest"
  }

  getBooks(kniga, page, sort)
})
